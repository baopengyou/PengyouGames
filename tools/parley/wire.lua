...
-- opens on Ki'katal (the journal's THIRD boss), walls there, and clears the
...
--
-- Ara-Kara (391): 2926 Avanoxx, 2906 Anub'zekt, 2900 Ki'katal the Harvester
local CARD = "T,X.6,L,O.2900,P.2926.1"
--   1  T          timed?                 -> Y
--   2  X.6        deaths over/under 6    -> 9 deaths, so OVER
--   3  L          first wall (boss pick) -> 2900, the LAST boss in the journal
--   4  O.2900     Ki'katal one-shot?     -> 3 attempts, so NO
--   5  P.2926.1   Avanoxx attempts o/u 1 -> 1 attempt, and OVER is strict: UNDER
local ROSTER = "2926~Avanoxx,2906~Anub'zekt,2900~Ki'katal the Harvester"
local BOSSDATA = "2900.3.5,2926.1.2,2906.1.1"   -- id.attempts.deaths, route order
...
check("3 First wall: Ki'katal (2900) wins even though it is the LAST boss",
...
check("4 Ki'katal one-shot?: 3 attempts, so NO wins and Ann alone pays",
...
check("5 Avanoxx attempts: 1 attempt is not OVER a line of 1, so UNDER wins",
...
opens("a duplicate per-boss line is refused", "O.2900,O.2900", false)
...
opens("two different bosses of the same type are fine", "O.2900,O.2926", true)
...
local FULL_CARD = "T,L,O.2900,P.2926.1,D.2906.2"
local FULL_PICKS = {
  { "Y", "N" },            -- T      timed?
  { "2900", "2926" },      -- L      first wall
  { "Y", "N" },            -- O.2900 one-shot?
  { "O", "U" },            -- P.2926 attempts
  { "O", "U" },            -- D.2906 deaths
}
local GOOD = { "C", "Y", "9", "3", "300", "T", "2900", "2900", BOSSDATA }
...
winner("Time left: exactly on the line is UNDER", "M.5", { { "O", "U" } },
  { "C", "Y", "9", "3", "300", "T", "2900", "2900", BOSSDATA }, "Cid-R")
winner("Time left: 400s beats a five-minute line", "M.5", { { "O", "U" } },
  { "C", "Y", "9", "3", "400", "T", "2900", "2900", BOSSDATA }, "Bob-R")
winner("Time left: an over-time key is UNDER any line", "M.5", { { "O", "U" } },
  { "C", "N", "9", "3", "-120", "T", "2900", "2900", BOSSDATA }, "Cid-R")
winner("First death: the tank died first", "F", { { "T", "H" } }, GOOD, "Bob-R")
winner("Worst boss: Ki'katal took the most attempts", "W",
  { { "2900", "2926" } }, GOOD, "Bob-R")
winner("First wall: Ki'katal, the journal's LAST boss", "L",
  { { "2926", "2900" } }, GOOD, "Cid-R")
...
winner("Boss deaths: Ki'katal's five beats a line of 2", "D.2900.2",
  { { "O", "U" } }, GOOD, "Bob-R")
winner("Boss one-shot: Avanoxx went down first try", "O.2926",
  { { "Y", "N" } }, GOOD, "Bob-R")
...
H.ME = "Ann-R"
recv(S, "OPEN", "ts1", BOOKIE, "group", 100, 391, "T,O.2900", "P")
...
H.ME = "Ann-R"
recv(S2, "OPEN", "ts2", BOOKIE, "group", 100, 391, "T,O.2900", "P")
